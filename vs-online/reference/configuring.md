...
title: Configuring Visual Studio Codespaces
ms.topic: overview
ms.date: 02/25/2020
---
...
Visual Studio Codespaces's [environments](../overview/what-is-vsonline.md#environments) are fully customizable on a per project basis. This is accomplished by including a `devcontainer.json` file in the project's repository.
...
## Visual Studio Codespaces configuration reference

The following tables lists the configuration properties supported by Codespaces. All properties are optional, unless otherwise noted.
...
| `context` | string | Path that the Codespaces's `docker build` command should be run from, relative to `devcontainer.json`. For example, a value of `".."` would allow you to reference content in sibling directories. Defaults to `"."`. |
...
## Visual Studio Codespaces configuration sample
